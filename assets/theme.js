  ...
  theme.Variants = (function() {
    ...
    Variants.prototype = Object.assign({}, Variants.prototype, {
      ...
      _updatePrice: function(variant) {
        ...
        // const alhPriceSpan = this.container.querySelector('[data-pro-price]');
        // if (alhPriceSpan) {
        //   alhPriceSpan.setAttribute('data-pro-price', variant.price);
        // }

        const alhPriceSpan = document.querySelectorAll('[data-pro-price]');
        if (alhPriceSpan){
          alhPriceSpan.forEach(span => {
            span.setAttribute('data-pro-price', variant.price);
          });
        }
  ...
  class ProductRecommendations extends HTMLElement {
    constructor() {
      ...
      document.addEventListener('recommendations:loaded', (e) => {
        ...
        // ✅ Attach add-to-cart listeners for recommendation products
    ...
    // ✅ New method for Add to Cart buttons
    initAddToCartButtons() {
      this.el.querySelectorAll(".alh-pro_addToCart").forEach(btn => {
        btn.addEventListener("click", function () {
          const variantId = this.dataset.variantId;
          const plueImage = btn.querySelector("img");

          btn.classList.add("btn--loading");
          if (plueImage) plueImage.style.display = "none";
          ...
          fetch("/cart/add.js", {
            ...
            .finally(() => {
              ...
              if (plueImage) plueImage.style.display = "block";
            });
        });
      });
    }
...
document.addEventListener('DOMContentLoaded', function () {
  document.querySelectorAll(".alh-pro_addToCart").forEach(function (btn) {
    btn.addEventListener("click", function () {
      const variantId = this.dataset.variantId;
      const plueImage = btn.querySelector("img");

      btn.classList.add("btn--loading");
      plueImage.style.display = "none";
      ...
      .then(response => response.json())
      .then(data => {
        // ✅ Fire custom event so the cart drawer opens
  ...
  wishlistButtons.forEach(button => {
    button.addEventListener('click', function () {
      ...
      fetch(`https://rfcproductionserver.vercel.app/api/v1/auth/wishlist/${customerId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      .then(res => res.json())
      .then(data => {
        console.log('Wishlist success:', data);
        button.disabled = true;
      })
      .catch(err => {
        console.error('Wishlist error:', err);
        alert('There was an error adding to wishlist. Please try again.');
      });
    });
  });
  ...
  const selectedValue = urlParams.get('per_page') || '16';

  // Set dropdown selected value
  perPageSelect.value = selectedValue;
